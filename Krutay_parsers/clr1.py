...
# -------------------------------
# 5. Pretty Print the Parsed Table
# -------------------------------

def print_parsing_table(action, goto_table, productions, grammar, augmented_start):
    ...
    print("\n=== PARSING TABLE ===")
    print('\t'.join(header))
    num_states = max(max(action.keys(), default=0), max(goto_table.keys(), default=0)) + 1
    for state in range(num_states):
        row = [f"{state}"]
        for sym in terminals:
            a = action[state].get(sym)
            if not a:
                row.append("")
            elif a[0] == "shift":
                row.append(f"s{a[1]}")
        ...
        for sym in non_terminals:
            row.append(str(goto_table[state].get(sym, "")))
        print('\t'.join(row))

# -------------------------------
# 6. Simulate CLR(1) Shift/Reduce Parsing
# -------------------------------
...
print_parsing_table(action, goto_table, productions, grammar, augmented_start)

# Print original production list
print("\n=== PRODUCTIONS ===")
